...
def json_to_word(json_data, output_filename="BUKU NYANYIAN HKBP HURUF BESAR2.docx"):
    ...
    run = paragraph.add_run("Buku Nyanyian HKBP")
    run.font.size = Pt(22)
    ...
    for lagu in json_data:
        # Menambahkan judul lagu dengan format "BN NO. {nomorBN} \"{judul}\"" dalam uppercase
        doc.add_heading(f"BN NO. {lagu['nomorBN']} \"{lagu['judul'].upper()}\"", level=1)  # Menambahkan heading
        
        # Menambahkan judul asli jika ada, tetap dalam format asli
        doc.add_paragraph(f"Judul Asli: {lagu.get('judul_asli', 'Tidak tersedia')}")
        
        # Menambahkan nada dasar jika ada, tetap dalam format asli
        ...
        for bait in lagu.get("baits", []):
            ...
            # Tambahkan satu baris kosong setelah setiap bait
            doc.add_paragraph("")  # Jarak antar bait
